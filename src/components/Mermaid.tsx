...
// Helper to preprocess chart string to fix common LLM syntax errors



const preprocessChart = (chart: string): string => {
  ...
  // 1. Fix spaces in arrows (e.g., " -- > ", " - > ")



  // We use regex to find these patterns ensuring they look like arrows
  ...
  // 2. Quote node labels containing curly braces if not already quoted.



  // Targets: id[Content {with} braces] -> id["Content {with} braces"]
  ...
  // 3. Quote node labels containing round braces (if in square brackets or round braces)
  ...
  useEffect(() => {
    ...
    const renderChart = async () => {
      ...
      try {
        ...
        // Preprocess the chart to fix common syntax errors
